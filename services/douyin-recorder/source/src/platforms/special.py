    ...
    async def fetch_stream(self, url: str, quality: str, proxy: Optional[str],
        ...
        with semaphore:
            if 'v.douyin.com' not in url and '/user/' not in url:
                json_data = await spider.get_douyin_web_stream_data(url=url, proxy_addr=proxy, cookies=dy_cookie)
            else:
                json_data = await spider.get_douyin_app_stream_data(url=url, proxy_addr=proxy, cookies=dy_cookie)
    ...
    async def fetch_stream(self, url: str, quality: str, proxy: Optional[str],
        ...
        with semaphore:
            if quality in ['OD', 'BD', 'UHD']:
                port_info = await spider.get_huya_app_stream_url(url=url, proxy_addr=proxy, cookies=hy_cookie)
            else:
                json_data = await spider.get_huya_stream_data(url=url, proxy_addr=proxy, cookies=hy_cookie)
...
class SOOPStrategy(PlatformStrategy):
    ...
    async def fetch_stream(self, url: str, quality: str, proxy: Optional[str],
        ...
        if not (global_proxy or proxy):
            logger.error("错误信息: 网络异常，无法访问SOOP")
            return None
        ...
        with semaphore:
            json_data = await spider.get_sooplive_stream_data(url=url, proxy_addr=proxy, cookies=sooplive_cookie,
...
class FlexTVStrategy(PlatformStrategy):
    ...
    async def fetch_stream(self, url: str, quality: str, proxy: Optional[str],
        ...
        with semaphore:
            json_data = await spider.get_flextv_stream_data(url=url, proxy_addr=proxy, cookies=flextv_cookie,
...
class PopkonTVStrategy(PlatformStrategy):
    ...
    async def fetch_stream(self, url: str, quality: str, proxy: Optional[str],
        ...
        with semaphore:
            port_info = await spider.get_popkontv_stream_url(url=url, proxy_addr=proxy, cookies=popkontv_cookie,
                username=popkontv_username, password=popkontv_password, partner_code=popkontv_partner_code)
...
class TwitCastingStrategy(PlatformStrategy):
    ...
    async def fetch_stream(self, url: str, quality: str, proxy: Optional[str],
        ...
        with semaphore:
            json_data = await spider.get_twitcasting_stream_url(url=url, proxy_addr=proxy, cookies=twitcasting_cookie,
                account_type=twitcasting_account_type, username=twitcasting_username, password=twitcasting_password)
...
class ShopeeStrategy(PlatformStrategy):
    ...
    async def fetch_stream(self, url: str, quality: str, proxy: Optional[str],
        ...
        with semaphore:
            port_info = await spider.get_shopee_stream_url(url=url, proxy_addr=proxy, cookies=shopee_cookie)
...
class TaobaoStrategy(PlatformStrategy):
    ...
    async def fetch_stream(self, url: str, quality: str, proxy: Optional[str],
        ...
        with semaphore:
            json_data = await spider.get_taobao_stream_url(url=url, proxy_addr=proxy, cookies=taobao_cookie)
            port_info = await stream.get_stream_url(json_data, quality, url_type='all', hls_extra_key='hlsUrl', flv_extra_key='flvUrl')
        return port_info
